...
token = "fa1vzn2pao9raxc17q7uto37ukterci6"


def Mkt_Price_Update():
    global mkt_data
    mkt_file = config.BaseDir_W + "Data\\" + config.LiveDataFile
    try:
        with open(mkt_file, 'r') as f:
            text = f.read()
            for item in text.split(','):
                stk,prc = item.split(':')
                # Exit if MArket data is older than 2 minutes.
                if stk == "TimeStamp":
                    tm = time.strptime(prc, "%Y%m%d%H%M%S")
                    delta = time.time() - time.mktime(tm)
                    if delta  > config.MarketDataDelayTolerance:
                        lg.debug("CT %s, TS %s, DIFF %s",   time.strftime("%Y%m%d%H%M%S"),  time.strftime("%Y%m%d%H%M%S", tm),  delta)
                        lg.error("TimeStamp in LiveData file is too old. Exiting. ")
                        exit()
                    else:
                        continue
                mkt_data[stk] = float(prc)
    except Exception as e:
        lg.error("Market Data File reading error. Exiting.")
        exit()
...
class MyKite:
    ...
    def order(self,scrip,bs,qty):
        try:     
            ...
            lg.error("Kite Order placement failed: %s", e.message)    
        return(order_id)
    ...
    def get_instr(self):
        try:
            ...
            lg.error("Kite Get instruments failed: %s", e.message)

    def order_cancle(self,order_id):
        try:
            self.kite.order_cancel(order_id="order_id")
        except Exception as e:
            lg.error("Kite Order %s cancle failed: %s",order_id, e.message)

    def get_orders(self):
        try:
            return(self.kite.orders())
        except Exception as e:
            lg.error("Kite Get Order status failed: %s",e.message)
...
if __name__ == "__main__":
    ...
    update_orders(kiteobj,orders)
    # write order book live
    update_order_book(orders,Stocks,"Live")
    # write order book close
    update_order_book(orders,Stocks,"Close")
