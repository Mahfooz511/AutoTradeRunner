...
class MarketData( ):
    ...
    def __init__(self,instruments,session_id):
        ...
        self.kws.on_data    = self.on_connect
    ...
    def on_data(self):
        print("Heart is beating. ", time.strftime("%H:%M:%S"))
    ...
    def on_tick(self,tick, ws):   
        ...
        for stock,price in self.stock_price.items():
            self.var_string += stock + ":" + str(price) + ','   
        ...
        if time.time() - self.time_counter  >= config.MarketData_Write_Interval:
            try:
                ...
                with open(self.publish_file, 'w') as f:
                    f.write("TimeStamp:" + time.strftime("%Y%m%d%H%M%S") + ',' + self.var_string[:-1])
                    print(time.strftime("%H:%M:%S"),self.var_string[:-1])
                self.time_counter = time.time()
...
# Start Getting Live market data. 
